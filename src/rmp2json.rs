...
fn utils_read_str(rd: &mut Bytes) -> Result<String, ()> {
    if let Ok(len) = read_str_len(rd) {
        let str = (0..len)
            .map(|_| rd.read_u8().map_err(err_handle))
            .collect::<Result<Vec<u8>, _>>()?;
        Ok(String::from_utf8(str).map_err(err_handle)?)
    } else {
        Err(())
    }
}
...
pub fn read_value(rd: &mut Bytes) -> Result<Value, ()> {
    ...
    let r = match m {
        ...
        F32 => Value::Number(
            Number::from_f64(rd.read_data_f32().map_err(err_handle)?.into()).ok_or(())?,
        ),
        F64 => Value::Number(Number::from_f64(rd.read_data_f64().map_err(err_handle)?).ok_or(())?),
        FixStr(size) => {
            let size = size as usize;
            let str = (0..size)
                .map(|_| rd.read_u8().map_err(err_handle))
                .collect::<Result<Vec<u8>, _>>()?;
            Value::String(String::from_utf8(str).map_err(err_handle)?)
        }
        Str8 => {
            let size = rd.read_data_u8().map_err(err_handle)? as usize;
            let str = (0..size)
                .map(|_| rd.read_u8().map_err(err_handle))
                .collect::<Result<Vec<u8>, _>>()?;
            Value::String(String::from_utf8(str).map_err(err_handle)?)
        ...
        Str16 => {
            let size = rd.read_data_u16().map_err(err_handle)? as usize;
            let str = (0..size)
                .map(|_| rd.read_u8().map_err(err_handle))
                .collect::<Result<Vec<u8>, _>>()?;
            Value::String(String::from_utf8(str).map_err(err_handle)?)
        ...
        Str32 => {
            let size = rd.read_data_u32().map_err(err_handle)? as usize;
            let str = (0..size)
                .map(|_| rd.read_u8().map_err(err_handle))
                .collect::<Result<Vec<u8>, _>>()?;
            Value::String(String::from_utf8(str).map_err(err_handle)?)
        ...
        FixMap(size) => {
            let size = size as usize;
            let map = (0..size)
                .map(|_| -> Result<(String, Value), ()> {
                    Ok((utils_read_str(rd)?, read_value(rd)?))
                })
